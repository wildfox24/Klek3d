...
size_z = (magnet_h + first_layer_height + (layers-1)*layer_height)*3;

module SD(){
	translate([-offset_x/2,-offset_y/2,2])
    cube([24+offset_x,64+offset_y,size_z]);
    //cube([25,25,30]);
	...
	difference(){
		...
		if (type == 2){
			if (slot1 == 1)
                translate([2.5,13,0])
                SD();
			//else if (slot1 == 2) translate([2.5,13,size_z-2.5]) SD();
			//if (slot2 == 1) translate([2.5,47,2.5]) SD();
			//else if (slot2 == 2) translate([2.5,47,2.5]) SD();
